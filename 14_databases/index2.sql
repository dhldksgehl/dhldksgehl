...
INSERT INTO customer (id, name, birthday) VALUES ('ccc', '이혜진', '1990-05-31');
...
-- 기준 테이블 : 기본키를 갖는 테이블
-- 참조 테이블 : 외래키가 있는 테이블
-- 형식 : FOREIGN KET () REFERENCES 기준_테이블(열_이름)
...
INSERT INTO orderlist (customer_id, product_name, quantity) VALUES ('ddd', '초코파이', 5);
INSERT INTO orderlist (customer_id, product_name, quantity) VALUES ('eee', '귀여운 텀블러', 1);
...
-- 하나의 큰 테이블(customer_id, product_name, quantity, name, bithday)
...
FROM customer
  ...
  -- 유저아이디를 기준으로 customr와 orderlist 조언 (일부 컬럼 이름에 별명을 붙여 모든 행 보기)
SELECT orderlist.id as order_id, customer.id as user_id, orderlist.prodct_name, orderlist.quantity
FROM customer
	INNER JOIN orderlist
    ON customer.id = orderlist.customer_id

-- 유저아이디를 기준으로 customr와 orderlist 조언 (일부 컬럼 이름에 별명을 붙여 조건을 만족하는 행 보기)
SELECT orderlist.id as order_id, customer.id as user_id, orderlist.prodct_name, orderlist.quantity
FROM customer
	INNER JOIN orderlist
    ON customer.id = orderlist.customer_id
